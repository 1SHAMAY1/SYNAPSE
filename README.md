...
### 1. Massively Parallel Swarm Orchestration
...
### 2. Consolidated Knowledge Engine
...
### 3. Cross-Modal Schematic Analysis
...
- **SQLite3**: Standard system dependency (provided via `better-sqlite3`).
...
2. Navigate to the **Settings** tab via the sidebar.
3. Register your API Providers:
...
4. Set your **Default Provider** to initialize the command station.
...
- **Standard Boot**: Run `SYNAPSE.bat` from the root directory.
- **Development Mode**: Execute `npm run dev` for hot-reloading and debug telemetry.

### Operational Workflow
1. **Initialize Session**: Click the **(+) Plus** button on the Session Rail to spawn a new tactical context.
2. **Objective Entry**: Input your core task into the command line. The Coordinator will automatically analyze the complexity and recruit a swarm.
3. **Active Steering**: You can interject into a running swarm at any time to redirect specialists or provide supplementary data.
4. **Context Consolidation**: Once a session reaches a high density, the system will automatically prompt to consolidateraw logs into a **Knowledge Node** for long-term optimization.
